...
이 문서는 `01_mcp_overview`와 `02_llm_mcp_integration`에서 다룬 내용을 **고급 구현 전략**으로 확장합니다.  
...
이 부분은 `02_llm_mcp_integration`의 “Tool 목록 로드” 흐름이 **어떤 정보를 기반으로 작동하는지**를 설명합니다.  
...
이 섹션은 `01_mcp_overview`에서 정의한 “도구 카드” 개념을 **실제 필드 구조**로 확장한 것입니다.
...
이 섹션은 `02_llm_mcp_integration`의 “실전 연동”을 운영 관점으로 확장합니다.  
